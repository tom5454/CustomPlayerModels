...
public class PlayerModelSetup {
	public static void setAngles(PlayerEntityModel<AbstractClientPlayerEntity> player, float f, float g, float i, float j, Arm arm, boolean swim) {
		...
		boolean bl4 = bl3 ? player.leftArmPose.isTwoHanded() : player.rightArmPose.isTwoHanded();
		if (bl3 != bl4) {
			method_30155(player);
			method_30154(player);
		} else {
		...
		if (player.sneaking) {
			...
			player.body.pivotY = 0.0F;
			player.leftArm.pivotY = 2.0F;
			player.rightArm.pivotY = 2.0F;
		}

		CrossbowPosing.swingArms(player.rightArm, player.leftArm, 0);
	...
	protected static void method_29353(PlayerEntityModel<AbstractClientPlayerEntity> player, Arm arm) {
		...
		player.body.yaw = MathHelper.sin(MathHelper.sqrt(g) * 6.2831855F) * 0.2F;
		if (arm == Arm.LEFT) {
			player.body.yaw *= -1.0F;
		}
		player.rightArm.pivotZ = MathHelper.sin(player.body.yaw) * 5.0F;
		player.rightArm.pivotX = -MathHelper.cos(player.body.yaw) * 5.0F;
		player.leftArm.pivotZ = -MathHelper.sin(player.body.yaw) * 5.0F;
		player.leftArm.pivotX = MathHelper.cos(player.body.yaw) * 5.0F;
		player.rightArm.yaw += player.body.yaw;
		player.leftArm.yaw += player.body.yaw;
		player.leftArm.pitch += player.body.yaw;
		...
		modelPart.yaw += player.body.yaw * 2.0F;
