...
public class PlayerRenderManager implements IPlayerRenderManager {
	...
	private static class RedirectHolder {
		...
		public RedirectHolder(PlayerEntityModel<AbstractClientPlayerEntity> model) {
			...
			bipedBody         = new RedirectModelRenderer(this, () -> model.body        , PlayerModelParts.BODY);
			...
			bipedHeadwear     = new RedirectModelRenderer(this, () -> model.hat    , null);
			...
			bipedLeftLegwear  = new RedirectModelRenderer(this, () -> model.leftPants , null);
			bipedRightLegwear = new RedirectModelRenderer(this, () -> model.rightPants, null);
		...
		public void swapIn(ModelDefinition def, Predicate<Object> unbindRule, VertexConsumerProvider buf) {
			...
			model.hat     = bipedHeadwear    .swapIn();
			model.leftSleeve  = bipedLeftArmwear .swapIn();
			model.rightSleeve = bipedRightArmwear.swapIn();
			model.leftPants  = bipedLeftLegwear .swapIn();
			model.rightPants = bipedRightLegwear.swapIn();
		...
		public void swapOut() {
			...
			model.hat     = bipedHeadwear    .swapOut();
			model.leftSleeve  = bipedLeftArmwear .swapOut();
			model.rightSleeve = bipedRightArmwear.swapOut();
			model.leftPants  = bipedLeftLegwear .swapOut();
			model.rightPants = bipedRightLegwear.swapOut();
	...
	private static class RedirectModelRenderer extends ModelPart {
		private static final String ID = "cpm:RedirectModelRenderer";
		...
		private static void translateRotate(RenderedCube rc, MatrixStack matrixStackIn) {
			...
			if (rc.rotation.z != 0.0F) {
				matrixStackIn.multiply(net.minecraft.util.math.Vec3f.POSITIVE_Z.getRadialQuaternion(rc.rotation.z));
			}

			if (rc.rotation.y != 0.0F) {
				matrixStackIn.multiply(net.minecraft.util.math.Vec3f.POSITIVE_Y.getRadialQuaternion(rc.rotation.y));
			}

			if (rc.rotation.x != 0.0F) {
				matrixStackIn.multiply(net.minecraft.util.math.Vec3f.POSITIVE_X.getRadialQuaternion(rc.rotation.x));
			}
