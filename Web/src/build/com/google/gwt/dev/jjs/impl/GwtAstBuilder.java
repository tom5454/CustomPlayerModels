...
import com.tom.cpm.web.gwt.SourceOriginPatch;
...
public class GwtAstBuilder {
	...
	class AstVisitor extends SafeASTVisitor {
		...
		public void endVisit(AND_AND_Expression x, BlockScope scope) {
			pushBinaryOp(x, JBinaryOperator.AND, scope);
		}
		...
		public void endVisit(Assignment x, BlockScope scope) {
			pushBinaryOp(x, JBinaryOperator.ASG, scope);
		}
		...
		public boolean visit(ReferenceExpression x, BlockScope blockScope) {
			...
			if (x.isArrayConstructorReference()) {
				...
				if (synthMethod.getBody() == null) {
					...
					List<JExpression> dims = new ArrayList<>();
		...
		public boolean visit(LambdaExpression x, BlockScope blockScope) {
			...
			SourceInfo info = makeSourceInfo(x, blockScope);
		...
		public void endVisit(OR_OR_Expression x, BlockScope scope) {
			pushBinaryOp(x, JBinaryOperator.OR, scope);
		}
		...
		public void endVisit(ReferenceExpression x, BlockScope blockScope) {
			...
			List<JField> enclosingInstanceFields = new ArrayList<>();
		...
		public void endVisit(SuperReference x, BlockScope scope) {
			try {
				...
				push(makeThisRef(makeSourceInfo(x, scope)));
			} catch (Throwable e) {
				throw translateException(x, e);
			}
		}
		...
		protected void pushBinaryOp(Assignment x, JBinaryOperator op, BlockScope scope) {
			pushBinaryOp(x, op, x.lhs, x.expression, scope);
		}

		protected void pushBinaryOp(BinaryExpression x, JBinaryOperator op, BlockScope scope) {
			pushBinaryOp(x, op, x.left, x.right, scope);
		}
		...
		private JLocal createLocal(LocalDeclaration x, BlockScope scope) {
		...
		private void pushBinaryOp(Expression x, JBinaryOperator op, Expression lhs, Expression rhs, BlockScope scope) {
			try {
				JType type = typeMap.get(x.resolvedType);
				SourceInfo info = makeSourceInfo(x, scope);
	...
	SourceInfo makeSourceInfo(ASTNode x, BlockScope scope) {
		return SourceOriginPatch.makeSourceInfo(x, curCud, sourceMapPath, scope);
	}
