...
class Consumer {
	...
	private void parse(String sourceMapData) {
		Map<String, Object> sourceMapRoot = (Map<String, Object>) new Gson().fromJson(sourceMapData, Object.class);
		...
		sourceFileNames = (List<String>) sourceMapRoot.get("sources");
		sourceSymbolNames = (List<String>) sourceMapRoot.get("names");

		lines = new ArrayList<>();

		new MappingBuilder((String) sourceMapRoot.get("mappings")).build();
	}
	...
	private class MappingBuilder
	{
		...
		void build()
		{
			...
			ArrayList<Mapping> entries = new ArrayList<>();
		...
		private Mapping decodeEntry(int generatedLine, int[] vals, int entryValues) {
			...
			case 5:
				...
				sourceSymbolName = sourceSymbolNames.get(sourceSymbolNameIndex);//PATCH
