...
public class ChunkSeqReaderPng extends ChunkSeqReader {
	...
	private long bytesAncChunksLoaded = 0; // bytes loaded from buffered chunks
											// non-critical chunks (data only)
	...
	private void updateAndCheckChunkGroup(String id) {
		if (id.equals(PngChunkIHDR.ID)) { // IDHR
			if (currentChunkGroup < 0) currentChunkGroup = ChunksList.CHUNK_GROUP_0_IDHR;
			else throw new PngjInputException("unexpected chunk " + id);
		} else if (id.equals(PngChunkPLTE.ID)) { // PLTE
			if ((currentChunkGroup == ChunksList.CHUNK_GROUP_0_IDHR
					|| currentChunkGroup == ChunksList.CHUNK_GROUP_1_AFTERIDHR))
				currentChunkGroup = ChunksList.CHUNK_GROUP_2_PLTE;
			else throw new PngjInputException("unexpected chunk here " + id);
		} else if (id.equals(PngChunkIDAT.ID)) { // IDAT (no necessarily the
													// first)
			if ((currentChunkGroup >= ChunksList.CHUNK_GROUP_0_IDHR
			...
			if ((currentChunkGroup >= ChunksList.CHUNK_GROUP_4_IDAT)) currentChunkGroup = ChunksList.CHUNK_GROUP_6_END;
			else throw new PngjInputException("unexpected chunk " + id);
		} else { // ancillary
			...
			else currentChunkGroup = ChunksList.CHUNK_GROUP_5_AFTERIDAT;
		}
	}
	...
	public boolean shouldSkipContent(int len, String id) {
		...
		if (ChunkHelper.isCritical(id)) return false;// critical chunks are
														// never skipped
		if (skipChunkMaxSize > 0 && len > skipChunkMaxSize) return true; // too
																			// big
																			// chunk
		if (maxBytesMetadata > 0 && len > maxBytesMetadata - bytesAncChunksLoaded) return true; // too
																								// much
																								// ancillary
																								// chunks
																								// loaded
		...
		case LOAD_CHUNK_IF_SAFE:
			if (!ChunkHelper.isSafeToCopy(id)) return true;
			break;
	...
	protected void postProcessChunk(ChunkReader chunkR) {
		// PngHelperInternal.debug("postProcessChunk " +
		// chunkR.getChunkRaw().id);
		...
		if (chunkR.getChunkRaw().id.equals(PngChunkIHDR.ID)) {
			...
			if (ch.isInterlaced()) deinterlacer = new Deinterlacer(curImageInfo);
	...
	@Override
	public void close() {
		if (currentChunkGroup != ChunksList.CHUNK_GROUP_6_END)// this could only
																// happen if
																// forced close
	...
	public void updateCurImgInfo(ImageInfo iminfo) {
		...
		if (deinterlacer != null) deinterlacer = new Deinterlacer(curImageInfo); // we
																					// could
																					// reset
																					// it,
																					// but...
