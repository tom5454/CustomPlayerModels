...
public class PngMetadata {
	...
	public double[] getDpi() {
		...
		if (c == null) return new double[] { -1, -1 };
		else return ((PngChunkPHYS) c).getAsDpi2();
	}
	...
	public PngChunkTextVar setText(String k, String val, boolean useLatin1, boolean compress) {
		if (compress && !useLatin1) throw new PngjException("cannot compress non latin text");
		PngChunkTextVar c;
		if (useLatin1) {
			if (compress) {
			...
			// ((PngChunkITXT) c).setTranslatedTag(k); // we use the same orig
			// tag (this is not quite right)
		}
		c.setKeyVal(k, val);
		queueChunk(c, true);
		return c;
