...
public class PixelsWriterDefault extends PixelsWriter {
	...
	protected int adaptSkipIncreaseSinceRow; // set in initParams, does not
												// change
	protected double adaptSkipIncreaseFactor; // set in initParams, does not
												// change
	...
	protected void initParams() {
		...
		if (rowb == null || rowb.length < buflen) rowb = new byte[buflen];
		if (rowbfilter == null || rowbfilter.length < buflen) rowbfilter = new byte[buflen];
		if (rowbprev == null || rowbprev.length < buflen) rowbprev = new byte[buflen];
		else Arrays.fill(rowbprev, (byte) 0);
		...
		if (FilterType.isAdaptive(filterType)) {
			...
			} else throw new PngjOutputException("bad filter " + filterType);
		}
	}

	@Override
	protected void filterAndWrite(final byte[] rowb) {
		if (rowb != this.rowb) throw new RuntimeException("??"); // we rely on
																	// this
		decideCurFilterType();
	...
	protected void decideCurFilterType() {
		...
		if (FilterType.isValidStandard(getFilterType())) {
			...
			if (currentRow == adaptNextRow) {
				...
				int skip = (currentRow >= adaptSkipIncreaseSinceRow
						? (int) Math.round((currentRow - adaptSkipIncreaseSinceRow) * adaptSkipIncreaseFactor)
						: 0);
		...
		if (currentRow == 0 && curfilterType != FilterType.FILTER_NONE && curfilterType != FilterType.FILTER_SUB)
			curfilterType = FilterType.FILTER_SUB; // first row should always be
													// none or sub
	}
