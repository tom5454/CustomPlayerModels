...
public class FiltersPerformance {
	...
	private double[] absum = new double[5];// depending on the strategy not all
											// values might be computed for all
	...
	private double preferenceForNone = 1.0; // higher gives more preference to
											// NONE

	// this values are empirical (montecarlo), for RGB8 images with entropy
	// estimator for NONE and memory=0.7
	// DONT MODIFY THIS
	// lower is better!
	...
	private void init() {
		if (filter_weights[0] < 0) {// has not been set from outside
			...
			if (iminfo.bitDepth == 16) wNone = 1.2;
			else if (iminfo.alpha) wNone = 0.8;
			else if (iminfo.indexed || iminfo.bitDepth < 8) wNone = 0.4; // we
																			// prefer
																			// NONE
																			// strongly
	...
	private void updateFromRawOrFiltered(FilterType ftype, byte[] rowff, byte[] rowb, byte[] rowbprev, int rown) {
		...
		lastrow = rown;
		if (rowff != null) computeHistogram(rowff);
		else computeHistogramForFilter(ftype, rowb, rowbprev);
		if (ftype == FilterType.FILTER_NONE) entropy[ftype.val] = computeEntropyFromHistogram();
		else absum[ftype.val] = computeAbsFromHistogram();
	}
