...
public class ImageInfo {
	...
	public ImageInfo(int cols, int rows, int bitdepth, boolean alpha, boolean grayscale, boolean indexed) {
		...
		if (greyscale && indexed) throw new PngjException("palette and greyscale are mutually exclusive");
		...
		if (cols < 1 || cols > MAX_COLS_ROW) throw new PngjException("invalid cols=" + cols + " ???");
		if (rows < 1 || rows > MAX_COLS_ROW) throw new PngjException("invalid rows=" + rows + " ???");
		if (samplesPerRow < 1) throw new PngjException("invalid image parameters (overflow?)");
	}
	...
	public long getTotalPixels() {
		if (totalPixels < 0) totalPixels = cols * (long) rows;
		return totalPixels;
	}
	...
	public long getTotalRawBytes() {
		if (totalRawBytes < 0) totalRawBytes = (bytesPerRow + 1) * (long) rows;
