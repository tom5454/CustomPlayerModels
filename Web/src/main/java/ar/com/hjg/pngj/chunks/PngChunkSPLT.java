...
public class PngChunkSPLT extends PngChunkMultiple {
	...
	public ChunkRaw createRawChunk() {
		try {
			...
			for (int n = 0; n < nentries; n++) {
				for (int i = 0; i < 4; i++) {
					if (sampledepth == 8) PngHelperInternal.writeByte(ba, (byte) palette[n * 5 + i]);
	...
	public void parseFromRaw(ChunkRaw c) {
		...
		if (t <= 0 || t > c.data.length - 2) throw new PngjException("bad sPLT chunk: no separator found");
		palName = ChunkHelper.toStringLatin1(c.data, 0, t);
