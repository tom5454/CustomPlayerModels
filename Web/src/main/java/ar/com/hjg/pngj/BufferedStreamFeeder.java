...
public class BufferedStreamFeeder implements Closeable {
	...
	private byte[] buf;
	private int pendinglen; // bytes read+stored in buf, not yet still sent to
							// IBytesConsumer
	private int offset;
	...
	public int feed(IBytesConsumer consumer, int maxbytes) {
		...
		if (tofeed > 0) {
			...
			if (!eof) throw new PngjInputException("This should not happen");
			return consumer.isDone() ? -1 : 0 /* premature ending */;
		}
		if (consumed > 0) {
			return consumed;
		} else { // read bytes, but consumer refused to eat them ? (rare)
			if (!consumer.isDone()) throw new PngjInputException("This should not happen!");
			return -1;
		}
	}
	...
	protected void refillBufferIfAppropiate() {
		...
		try {
			...
			else if (pendinglen < 0) close(); // this sets EOF and pendinglen=0
			else bytesRead += pendinglen;
		} catch (IOException e) {
	...
	public void close() {
		...
		if (stream != null && closeStream) {
			try {
				...
				// PngHelperInternal.LOGGER.log(Level.WARNING, "Exception
				// closing stream", e);
			}
		}
		stream = null;
