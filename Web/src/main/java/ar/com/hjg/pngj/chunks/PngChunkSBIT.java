...
public class PngChunkSBIT extends PngChunkSingle {
	...
	public void parseFromRaw(ChunkRaw c) {
		if (c.len != getCLen()) throw new PngjException("bad chunk length " + c);
		if (imgInfo.greyscale) {
			graysb = PngHelperInternal.readInt1fromByte(c.data, 0);
			if (imgInfo.alpha) alphasb = PngHelperInternal.readInt1fromByte(c.data, 1);
		} else {
			...
			if (imgInfo.alpha) alphasb = PngHelperInternal.readInt1fromByte(c.data, 3);
		}
	}
	...
	public ChunkRaw createRawChunk() {
		...
		if (imgInfo.greyscale) {
			...
			if (imgInfo.alpha) c.data[1] = (byte) alphasb;
		} else {
			c.data[0] = (byte) redsb;
			c.data[1] = (byte) greensb;
			c.data[2] = (byte) bluesb;
			if (imgInfo.alpha) c.data[3] = (byte) alphasb;
		}
		return c;
	}

	public void setGraysb(int gray) {
		if (!imgInfo.greyscale) throw new PngjException("only greyscale images support this");
		graysb = gray;
	}
	...
	public void setAlphasb(int a) {
		if (!imgInfo.alpha) throw new PngjException("only images with alpha support this");
		alphasb = a;
	}
	...
	public void setRGB(int r, int g, int b) {
		if (imgInfo.greyscale || imgInfo.indexed) throw new PngjException("only rgb or rgba images support this");
		redsb = r;
		greensb = g;
		bluesb = b;
	}
