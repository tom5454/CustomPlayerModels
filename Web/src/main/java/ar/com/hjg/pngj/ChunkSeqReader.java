...
import com.tom.cpm.web.client.java.io.FileInputStream;
...
public abstract class ChunkSeqReader implements IBytesConsumer, Closeable {
	...
	private DeflatedChunksSet curDeflatedSet; // one instance for each
												// "idat-like set". Normally
												// one.

	private ChunkReader curChunkReader;

	private long idatBytes; // this is only for the IDAT (not mrerely
							// "idat-like")
	...
	public int consume(byte[] buffer, int offset, int len) {
		...
		if (len < 0) throw new PngjInputException("This should not happen. Bad length: " + len);
		int processed = 0;
		if (signatureDone) {
			if (curChunkReader == null || curChunkReader.isDone()) { // new
																		// chunk:
																		// read
																		// first
																		// 8
	...
	protected void startNewChunk(int len, String id, long offset) {
		...
		if (len < 0) throw new PngjInputException("Bad chunk len: " + len);
		if (id.equals(ChunkHelper.IDAT)) idatBytes += len;
		...
		if (isIdatType && !skip) { // IDAT non skipped: create a
									// DeflatedChunkReader owned by a idatSet
	 ...
	 **/
	protected void postProcessChunk(ChunkReader chunkR) { // called after chunk
															// is read
		if (chunkCount == 1) {
			// first chunk ?
