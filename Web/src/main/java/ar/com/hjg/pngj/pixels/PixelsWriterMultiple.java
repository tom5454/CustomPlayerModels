...
import com.tom.cpm.web.client.java.zip.Deflater;
...
public class PixelsWriterMultiple extends PixelsWriter {
	...
	protected int rowsPerBandCurrent = 0; // lastRowInThisBand-firstRowInThisBand
											// +1 : might be smaller than
											// rowsPerBand
	...
	// we will consume about (not more than) this memory (in buffers, not
	// counting the compressors)
	protected int hintMemoryKb = HINT_MEMORY_DEFAULT_KB;

	private int hintRowsPerBand = 1000; // default: very large number, can be
										// changed
	...
	public PixelsWriterMultiple(ImageInfo imgInfo) {
		...
		for (int i = 0; i < 2; i++)
			rows.add(new byte[buflen]); // we preallocate 2 rows (rowb and
										// rowbprev)
	...
	protected void filterAndWrite(byte[] rowb) {
		if (!initdone) init();
		if (rowb != rows.get(0)) throw new RuntimeException("?");
		setBandFromNewRown();
		byte[] rowbprev = rows.get(1);
		for (FilterType ftype : FilterType.getAllStandardNoneLast()) {
			// this has a special behaviour for NONE: filteredRows[0] is null,
			// and the returned value is rowb
			...
			if (currentRow == 0 && ftype == FilterType.FILTER_SUB) { // litle
																		// lie,
																		// only
																		// for
																		// first
		...
		if (currentRow == lastRowInThisBand) {
			...
			// PngHelperInternal.debug("won: " + best + " (rows: " +
			// firstRowInThisBand +":" + lastRowInThisBand + ")");
			// if(currentRow>90&&currentRow<100)
			// PngHelperInternal.debug(com.tom.cpm.web.client.java.Java.format("row=%d
			// ft=%s",currentRow,FilterType.getByVal(best)));
			byte[] filtersAdapt = filterBank[best].getFirstBytes();
			for (int r = firstRowInThisBand, i = 0,
					j = lastRowInThisBand - firstRowInThisBand; r <= lastRowInThisBand; r++, j--, i++) {
		...
		} else rows.addFirst(new byte[buflen]);
	}
	...
	private void setBandFromNewRown() {
		...
		if (newBand) {
			...
			if (lastRowInNextBand >= imgInfo.rows) // hack:make this band
													// bigger, so we don't have
													// a small last band
	...
	private void rebuildFiltersBank() {
		...
		for (int i = 0; i <= 5; i++) {// one for each filter plus one adaptive
			...
			if (cp == null || cp.totalbytes != bytesPerBandCurrent) {
				if (cp != null) cp.close();
				if (useLz4) cp = new CompressorStreamLz4(null, buflen, bytesPerBandCurrent);
				else cp = new CompressorStreamDeflater(null, buflen, bytesPerBandCurrent, DEFLATER_COMP_LEVEL,
						Deflater.DEFAULT_STRATEGY);
				filterBank[i] = cp;
			} else {
				cp.reset();
			}
			cp.setStoreFirstByte(true, rowsPerBandCurrent); // TODO: only for
															// adaptive?
		}
	}
	...
	private int computeInitialRowsPerBand() {
		...
		if (r < 1) r = 1;
		if (hintRowsPerBand > 0 && r > hintRowsPerBand) r = hintRowsPerBand;
		if (r > imgInfo.rows) r = imgInfo.rows;
	...
	private int getBestCompressor() {
		...
		for (int i = tryAdaptive ? 5 : 4; i >= 0; i--) {
			...
			if (cr <= bestcr) { // dirty trick, here the equality gains for row
								// 0, so that SUB is prefered over PAETH, UP,
								// AVE...
				bestb = i;
				bestcr = cr;
			}
	...
	protected void initParams() {
		...
		for (int i = 1; i <= 4; i++) { // element 0 is not allocated
			if (filteredRows[i] == null || filteredRows[i].length < buflen) filteredRows[i] = new byte[buflen];
		}
		if (rowsPerBand == 0) rowsPerBand = computeInitialRowsPerBand();
	}
