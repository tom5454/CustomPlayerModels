...
public class PngWriter implements Closeable {
	...
	private int passes = 1; // Some writes might require two passes (NOT USED
							// STILL)
	...
	private void writeFirstChunks() {
		...
		nw = chunksList.writeChunks(os, currentChunkGroup);
		if (nw > 0 && imgInfo.greyscale) throw new PngjOutputException("cannot write palette for this format");
		if (nw == 0 && imgInfo.indexed) throw new PngjOutputException("missing palette");
	...
	private void queueChunksFromOther() {
		...
		boolean idatDone = currentChunkGroup >= ChunksList.CHUNK_GROUP_4_IDAT; // we
																				// assume
																				// this
																				// method
																				// is
		...
		for (PngChunk chunk : copyFromList.getChunks()) {
			if (chunk.getRaw().data == null) continue; // we cannot copy skipped
														// chunks?
			int groupOri = chunk.getChunkGroup();
			if (groupOri <= ChunksList.CHUNK_GROUP_4_IDAT && idatDone) continue;
			if (groupOri >= ChunksList.CHUNK_GROUP_4_IDAT && !idatDone) continue;
			if (chunk.crit && !chunk.id.equals(PngChunkPLTE.ID)) continue; // critical
																			// chunks
																			// (except
																			// perhaps
																			// PLTE)
	 ...
	 * {@link ar.com.hjg.pngj.chunks.ChunkHelper#equivalent(PngChunk, PngChunk)},
	 * this overwrites it.
	 * <p>
	...
	public void setFilterPreserve(boolean filterPreserve) {
		if (filterPreserve) pixelsWriter.setFilterType(FilterType.FILTER_PRESERVE);
		else if (pixelsWriter.getFilterType() == null) pixelsWriter.setFilterType(FilterType.FILTER_DEFAULT);
	}
	...
	public void writeRow(IImageLine imgline, int rownumber) {
		...
		if (rownumber == 0 && currentpass == passes) {
			initIdat();
			currentChunkGroup = ChunksList.CHUNK_GROUP_4_IDAT; // we just begin
																// writing IDAT
		}
