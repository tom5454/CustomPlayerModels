...
public class PngReaderApng extends PngReaderByte {
	...
	public boolean isApng() {
		if (apngKind == null) {
			...
			actlChunk = (PngChunkACTL) getChunksList().getById1(PngChunkACTL.ID); // null
																					// if
																					// not
																					// apng
	...
	public void advanceToFrame(int frame) {
		if (frame < frameNum) throw new PngjInputException("Cannot go backwards");
		if (frame >= getApngNumFrames()) throw new PngjInputException("Frame out of range " + frame);
		if (frame > frameNum) {
			addChunkToSkip(PngChunkIDAT.ID);
			addChunkToSkip(PngChunkFDAT.ID);
			if (chunkseq.getIdatSet() != null && !chunkseq.getIdatSet().isDone()) chunkseq.getIdatSet().markAsDone(); // seems
																														// to
																														// be
																														// necessary
																														// sometimes
		...
		if (frame == frameNum) { // prepare to read rows. at this point we have
									// a new
	...
	public int getApngNumFrames() {
		if (isApng()) return actlChunk.getNumFrames();
		else return 0;
	}
	...
	public int getApngNumPlays() {
		if (isApng()) return actlChunk.getNumPlays();
		else return -1;
	}
