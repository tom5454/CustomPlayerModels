...
import com.tom.cpm.web.client.java.zip.Checksum;
...
public class IdatSet extends DeflatedChunksSet {
	...
	protected final ImageInfo imgInfo; // in the case of APNG this is the frame
										// image
	 ...
	 * <p>
	 * <p>
	 * Same as {@link #IdatSet(String, boolean, ImageInfo, Deinterlacer)}, but
	 * you can pass a Inflater (will be reset internally), and a buffer (will be
	 * used only if size is enough)
	 */
	...
	protected void unfilterRow(int nbytes) {
		...
		if (rowinfo.rowNsubImg == 0) Arrays.fill(rowUnfiltered, (byte) 0); // see
																			// swap
																			// that
																			// follows
		// swap
		...
		int ftn = row[0];
		if (!FilterType.isValidStandard(ftn)) throw new PngjInputException("Filter type " + ftn + " invalid");
	...
	public int advanceToNextRow() {
		...
		if (!callbackMode) { // in callback mode, setNextRowLen() is called
								// internally
