...
public class PngChunkIHDR extends PngChunkSingle {
	...
	//
	public PngChunkIHDR(ImageInfo info) { // argument is normally null here, if
											// not null is used to fill the
											// fields
		super(ID, info);
	...
	public void parseFromRaw(ChunkRaw c) {
		if (c.len != 13) throw new PngjException("Bad IDHR len " + c.len);
	...
	public void check() {
		...
		if (interlaced < 0 || interlaced > 1) throw new PngjInputException("bad IHDR: interlace invalid");
		switch (colormodel) {
		case 0:
			break;
		case 3:
			if (bitspc == 16) throw new PngjInputException("bad IHDR: bitdepth invalid");
			break;
		case 2:
		case 4:
		case 6:
			if (bitspc != 8 && bitspc != 16) throw new PngjInputException("bad IHDR: bitdepth invalid");
			break;
		default:
