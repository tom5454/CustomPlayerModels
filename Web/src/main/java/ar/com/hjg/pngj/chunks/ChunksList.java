...
public class ChunksList {
	...
	public static final int CHUNK_GROUP_4_IDAT = 4; // required (single pseudo
													// chunk)
	...
	protected static List<PngChunk> getXById(final List<PngChunk> list, final String id, final String innerid) {
		if (innerid == null) return ChunkHelper.filterList(list, new ChunkPredicate() {
			public boolean match(PngChunk c) {
				return c.id.equals(id);
			}
		});
		else return ChunkHelper.filterList(list, new ChunkPredicate() {
			public boolean match(PngChunk c) {
				if (!c.id.equals(id)) return false;
				if (c instanceof PngChunkTextVar && !((PngChunkTextVar) c).getKey().equals(innerid)) return false;
				if (c instanceof PngChunkSPLT && !((PngChunkSPLT) c).getPalName().equals(innerid)) return false;
				return true;
			}
		});
	}
	...
	public void appendReadChunk(PngChunk chunk, int chunkGroup) {
		...
		chunks.add(chunk);
		if (chunk.id.equals(PngChunkPLTE.ID)) withPlte = true;
	}
