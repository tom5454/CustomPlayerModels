...
import com.tom.cpm.web.client.java.io.File;
import com.tom.cpm.web.client.java.zip.Adler32;
import com.tom.cpm.web.client.java.zip.CRC32;
...
public class PngReader implements Closeable {
	 ...
	 */
	public static final long MAX_BYTES_METADATA_DEFAULT = 5024024; // for
																	// ancillary
																	// chunks
	 ...
	 */
	public static final long MAX_CHUNK_SIZE_SKIP = 2024024; // chunks exceeding
															// this size will be
															// skipped (nor even
															// CRC
	// checked)
	 ...
	 */
	public final ImageInfo imgInfo; // People always told me: be careful what
									// you do, and don't go around declaring
	// public
	// fields...
	/**
	...
	public PngReader(InputStream inputStream, boolean shouldCloseStream) {
		...
		try {
			if (streamFeeder.feedFixed(chunkseq, 36) != 36) // 8+13+12=36 PNG
															// signature+IHDR
															// chunk
			...
			chunkseq.addChunkToSkip(PngChunkFDAT.ID);// default: skip fdAT
														// chunks!
			chunkseq.addChunkToSkip(PngChunkFCTL.ID);// default: skip fctl
														// chunks!
	 ...
	 * <code>shouldCloseStream=true</code>, so that the stream will be closed
	 * with this object.
	 * 
	 * @param file
	 *            PNG image file
	...
	public IImageLine readRow(int nrow) {
		...
		if (!interlaced) {
			...
			else if (nrow < rowNum) throw new PngjInputException("rows must be read in increasing order: " + nrow);
			while (rowNum < nrow) {
	...
	public IImageLineSet<? extends IImageLine> readRows(int nRows, int rowOffset, int rowStep) {
		...
		if (rowNum >= rowOffset) throw new PngjInputException("readRows cannot be mixed with readRow");
	...
	protected void loadAllInterlaced(int nRows, int rowOffset, int rowStep) {
		...
		do {
			...
			if (!chunkseq.getIdatSet().isRowReady()) throw new PngjInputException("Premature ending?");
			chunkseq.getIdatSet().updateCrcs(idatCrca, idatCrcb);
	...
	public void end() {
		try {
			if (chunkseq.firstChunksNotYetRead()) readFirstChunks();
			if (chunkseq.getIdatSet() != null && !chunkseq.getIdatSet().isDone()) chunkseq.getIdatSet().markAsDone(); // it
																														// will
																														// ignore
																														// data
	...
	public void close() {
		...
		if (streamFeeder != null) streamFeeder.close();
	}
	...
	public void prepareSimpleDigestComputation() {
		if (idatCrca == null) idatCrca = new CRC32();
		else idatCrca.reset();
		if (idatCrcb == null) idatCrcb = new Adler32();
		else idatCrcb.reset();
	...
	public String getSimpleDigestHex() {
		return com.tom.cpm.web.client.java.Java.format("%016X", getSimpleDigest());
	}
