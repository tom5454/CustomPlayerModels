...
public abstract class ImageLineSetDefault<T extends IImageLine> implements IImageLineSet<T> {
	...
	public ImageLineSetDefault(ImageInfo imgInfo, final boolean singleCursor, final int nlinesx, final int noffsetx,
			final int stepx) {
		this.imgInfo = imgInfo;
		this.singleCursor = singleCursor;
		if (singleCursor) {
			this.nlines = 1; // we store only one line, no matter how many will
								// be read
			offset = 0;
	...
	private void createImageLines() {
		if (singleCursor) imageLine = createImageLine();
		else {
	...
	public T getImageLine(int n) {
		...
		if (singleCursor) return imageLine;
			...
			if (r < 0) throw new PngjException("Invalid row number");
	...
	public static <T extends IImageLine> IImageLineSetFactory<T> createImageLineSetFactoryFromImageLineFactory(
			final IImageLineFactory<T> ifactory) { // ugly method must have ugly
													// name. don't let this
													// intimidate you
