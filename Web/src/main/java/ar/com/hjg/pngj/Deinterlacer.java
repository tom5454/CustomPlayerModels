...
public class Deinterlacer {
	...
	// current row in the virtual subsampled image; this increments (by 1) from
	// 0 to rows/dy 7 times
	private int currRowSubimg;
	// in the real image, this will cycle from 0 to im.rows in different steps,
	// 7 times
	...
	private void setRow(int n) { // This should be called only intercally, in
									// sequential order
		currRowSubimg = n;
		currRowReal = n * dY + oY;
	...
	public long getTotalRawBytes() { // including the filter byte
		...
		for (int p = 1; p <= 7; p++) {
			...
			if (rows > 0 && cols > 0) bytes += rows * (1 + (long) bytesr);
		}
		return bytes;
	}
