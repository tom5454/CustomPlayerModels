...
public class ImageLineHelper {
	...
	public static void scaleUp(IImageLineArray line) {
		if (line.getImageInfo().indexed || line.getImageInfo().bitDepth >= 8) return;
		...
		} else throw new PngjException("not implemented");
	}
	...
	static int[] lineToARGB32(ImageLineByte line, PngChunkPLTE pal, PngChunkTRNS trns, int[] buf) {
		...
		if (buf == null || buf.length < cols) buf = new int[cols];
	...
	public static String infoFirstLastPixels(ImageLineInt line) {
		return line.imgInfo.channels == 1
				? com.tom.cpm.web.client.java.Java.format("first=(%d) last=(%d)", line.scanline[0],
						line.scanline[line.scanline.length - 1])
				: com.tom.cpm.web.client.java.Java.format("first=(%d %d %d) last=(%d %d %d)", line.scanline[0],
						line.scanline[1], line.scanline[2], line.scanline[line.scanline.length - line.imgInfo.channels],
	...
	public static int getPixelARGB8(IImageLine line, int column) {
		...
		} else throw new PngjUnsupportedException("Not supported " + line.getClass());
	}
	...
	public static int getMaskForPackedFormats(int bitDepth) { // Utility
																// function for
																// pack/unpack
		if (bitDepth == 4) return 0xf0;
		else if (bitDepth == 2) return 0xc0;
		else return 0x80; // bitDepth == 1
	}

	public static int getMaskForPackedFormatsLs(int bitDepth) { // Utility
																// function for
																// pack/unpack
		if (bitDepth == 4) return 0x0f;
		else if (bitDepth == 2) return 0x03;
		else return 0x01; // bitDepth == 1
	}
