...
public abstract class PngChunk {
	...
	// For writing. Queued chunks with high priority will be written as soon as
	// possible
	...
	public enum ChunkOrderingConstraint {
		...
		public boolean isOk(int currentChunkGroup, boolean hasplte) {
			...
			else if (this == BEFORE_IDAT) return currentChunkGroup < ChunksList.CHUNK_GROUP_4_IDAT;
			else if (this == BEFORE_PLTE_AND_IDAT) return currentChunkGroup < ChunksList.CHUNK_GROUP_2_PLTE;
			else if (this == AFTER_PLTE_BEFORE_IDAT) return hasplte ? currentChunkGroup < ChunksList.CHUNK_GROUP_4_IDAT
					: (currentChunkGroup < ChunksList.CHUNK_GROUP_4_IDAT
							&& currentChunkGroup > ChunksList.CHUNK_GROUP_2_PLTE);
			else if (this == AFTER_IDAT) return currentChunkGroup > ChunksList.CHUNK_GROUP_4_IDAT;
			return false;
		}
	}
	...
	protected final ChunkRaw createEmptyChunk(int len, boolean alloc) {
		ChunkRaw c = new ChunkRaw(len, ChunkHelper.toBytesLatin1(id), alloc);
		return c;
	}
	...
	final void write(OutputStream os) {
		...
		if (raw == null) throw new PngjExceptionInternal("null chunk ! creation failed for " + this);
		raw.writeChunk(os);
	}
