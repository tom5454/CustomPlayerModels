...
import com.tom.cpm.web.client.java.zip.DataFormatException;
import com.tom.cpm.web.client.java.zip.Inflater;
...
public class DeflatedChunksSet {
	...
	protected byte[] row; // a "row" here means a raw (uncopressed filtered)
							// part of the IDAT stream,
	...
	private int rowlen; // what amount of bytes is to be interpreted as a
						// complete "row". can change
	// (for interlaced)
	private int rown; // only coincide with image row if non-interlaced -
						// incremented by
	// setNextRowSize()
	 ...
	 * ignored)
	 * 
	 * WARNING: inflater.finished() != DONE (not enough, not neccesary) DONE
	 * means that we have already uncompressed all the data of interest.
	 * 
	 ...
	 * 
	 * prepareForNextRow(n) goes to DONE if n==0 calls inflateData() again -
	 * end() goes to DONE
	 */
	private enum State {
		WAITING_FOR_INPUT, // waiting for more bytes to be fed
		ROW_READY, // ready for consumption (might be less than fully filled),
					// ephemeral state for
		// CALLBACK mode
		DONE, // all data of interest has been read, but we might accept still
				// more trailing
	...
	protected final boolean callbackMode; // true: CALLBACK (non-blocking)
											// false: POLL (blocking)

	private long nBytesIn = 0; // count the total compressed bytes that have
								// been fed
	private long nBytesOut = 0; // count the total uncompressed bytes
	int chunkNum = -1; // incremented at each new chunk start
	int firstChunqSeqNum = -1; // expected seq num for first chunk. used only
								// for fDAT (APNG)
	...
	protected void appendNewChunk(DeflatedChunkReader cr) {
		...
		if (!this.chunkid.equals(cr.getChunkRaw().id)) throw new PngjInputException(
	...
	protected void processBytes(byte[] buf, int off, int len) {
		nBytesIn += len;
		// PngHelperInternal.LOGGER.info("processing compressed bytes in
		// chunkreader : "
		// + len);
	...
	/*
	 * This never inflates more than one row This returns true if this has
	 * resulted in a row being ready and preprocessed with preProcessRow (in
	 * callback mode, we should call immediately processRowCallback() and
	 * prepareForNextRow(nextRowLen)
	 */
	private boolean inflateData() {
		try {
			// PngHelperInternal.debug("entering inflateData bytes, state=" +
			// state +
			// " callback="+callbackMode);
			if (state == State.ROW_READY) throw new PngjException("invalid state");// assert
			if (state.isDone()) return false;
			int ninflated = 0;
			if (row == null || row.length < rowlen) row = new byte[rowlen]; // should
																			// not
																			// happen
			...
			if (rowfilled == rowlen) nextstate = State.ROW_READY; // complete
																	// row,
																	// process
																	// it
			else if (!inf.finished()) nextstate = State.WAITING_FOR_INPUT;
			else if (rowfilled > 0) nextstate = State.ROW_READY; // complete
																	// row,
																	// process
																	// it
			else {
	...
	/**
	 * Forces the DONE state (except if it was CLOSED), this object won't
	 * uncompress more data. It's still not terminated, it will accept more IDAT
