...
public class ImageLineByte implements IImageLine, IImageLineArray {
	...
	final byte[] scanline2; // only used for 16 bpp (less significant byte)
							// Normally you'd prefer
	// ImageLineInt in this case

	protected FilterType filterType; // informational ; only filled by the
										// reader. not significant for
	// interlaced
	final int size; // = imgInfo.samplePerRowPacked, if
					// packed:imgInfo.samplePerRow elswhere
	...
	public void readFromPngRaw(byte[] raw, final int len, final int offset, final int step) {
		filterType = FilterType.getByVal(raw[0]); // only for non interlaced
													// line the filter is
													// significative
		int len1 = len - 1;
		int step1 = (step - 1) * imgInfo.channels;
		if (imgInfo.bitDepth == 8) {
			if (step == 1) {// 8bispp non-interlaced: most important case,
							// should be optimized
