...
public class ChunksListForWrite extends ChunksList {
	...
	private static boolean shouldWrite(PngChunk c, int currentGroup) {
		if (currentGroup == CHUNK_GROUP_2_PLTE) return c.id.equals(ChunkHelper.PLTE);
		if (currentGroup % 2 == 0) throw new PngjOutputException("bad chunk group?");
		...
		if (ChunkHelper.isUnknown(c) && c.getChunkGroup() > 0) preferred = c.getChunkGroup();
