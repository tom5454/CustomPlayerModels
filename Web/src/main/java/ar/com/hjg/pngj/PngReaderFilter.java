 ...
 * reader.readUntilEndAndClose(); // in case ImageIO.read() does not read the
 * 								// traling chunks (it happens)
 * </pre>
 * 
 */
public class PngReaderFilter extends FilterInputStream {
	...
	public int read() throws IOException {
		int r = super.read();
		if (r > 0) chunkseq.feedAll(new byte[] { (byte) r }, 0, 1);
		return r;
	}
	...
	public int read(byte[] b) throws IOException {
		...
		if (res > 0) chunkseq.feedAll(b, 0, res);
		return res;
	}
