...
public class CompressorStreamLz4 extends CompressorStream {
	...
	public CompressorStreamLz4(IdatChunkWriter os, int maxBlockLen, long totalLen, int deflaterCompLevel,
	...
	public void mywrite(byte[] b, int off, int len) {
		if (len == 0) return;
		if (done || closed) throw new PngjOutputException("write beyond end of stream");
		bytesIn += len;
		while (len > 0) {
			if (inbuf == 0 && (len >= MAX_BUFFER_SIZE || bytesIn == totalbytes)) {
				...
				int len1 = inbuf + len <= buffer_size ? len : buffer_size - inbuf; // to
																					// copy
				if (len1 > 0) System.arraycopy(b, off, buf, inbuf, len1);
				inbuf += len1;
				len -= len1;
				off += len1;
