...
public interface IBytesConsumer {
	...
	/**
	 * The consumer is DONE when it does not need more bytes, either because it
	 * ended normally, or abnormally Typically this implies it will return -1 if
	 * consume() is called afterwards, but it might happen that it will consume
	 * more (unneeded) bytes anwyway
	 */
	boolean isDone();
}
