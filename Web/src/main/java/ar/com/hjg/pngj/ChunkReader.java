 ...
 * {@link #processData(int, byte[], int, int)} if <code>PROCESS</code> mode.
 * Apart from thas, it's totally agnostic (it doesn't know about IDAT chunks, or
 * PNG general structure)
 * <p>
 * The object wraps a <code>ChunkRaw</code> instance (content allocated and
 * filled only if BUFFER mode). It should be short lived (one instance created
 * for each chunk, and discarded after reading), but the wrapped
 * <code>chunkRaw</code> can be (usually is) long lived.
 */
public abstract class ChunkReader implements IBytesConsumer {
	...
	private boolean crcCheck; // by default, this is false for SKIP, true
								// elsewhere
	...
	public ChunkReader(int clen, String id, long offsetInPng, ChunkReaderMode mode) {
		...
		this.crcCheck = mode == ChunkReaderMode.SKIP ? false : true; // can be
																		// changed
																		// with
																		// setter
		// PngHelperInternal.debug("ChunkReader " + this.getClass() + " id="+id
		// + " mode:"+mode);
	}
	...
	public final int consume(byte[] buf, int off, int len) {
		...
		if (read == 0 && crcn == 0 && crcCheck) chunkRaw.updateCrc(chunkRaw.idbytes, 0, 4); // initializes
																							// crc
																							// calculation
																							// with
																							// the
																							// Chunk
																							// ID
		int bytesForData = chunkRaw.len - read; // bytesForData : bytes to be
												// actually read from chunk data
		if (bytesForData > len) bytesForData = len;
		// we want to call processData even for empty chunks
		// (IEND:bytesForData=0) at
		// least once
		if (bytesForData > 0 || crcn == 0) {
			...
			if (mode == ChunkReaderMode.BUFFER) {
				...
				if (chunkRaw.data != buf && bytesForData > 0) {
					// If the buffer passed if the same as this one, we don't
					// copy.
		...
		if (read == chunkRaw.len) { // data done - read crc?
			...
			if (crcRead > 0) {
				if (buf != chunkRaw.crcval) System.arraycopy(buf, off, chunkRaw.crcval, crcn, crcRead);
				crcn += crcRead;
				if (crcn == 4) {
					if (crcCheck) {
						if (mode == ChunkReaderMode.BUFFER) { // in buffer mode
																// we compute
																// the CRC on
																// one single
																// call
		...
		return bytesForData > 0 || crcRead > 0 ? bytesForData + crcRead
				: -1 /* should not happen */ ;
	}
	...
	public void setCrcCheck(boolean crcCheck) {
		if (read != 0 && crcCheck && !this.crcCheck) throw new PngjException("too late!");
	...
	public boolean equals(Object obj) { // delegates to chunkraw
		...
		if (chunkRaw == null) {
			if (other.chunkRaw != null) return false;
		} else if (!chunkRaw.equals(other.chunkRaw)) return false;
		return true;
	}
