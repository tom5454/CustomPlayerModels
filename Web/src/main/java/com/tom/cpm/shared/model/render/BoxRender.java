...
public class BoxRender {
	...
	private static class Quad {
		...
		public Quad(Vertex[] posIn, float u1, float v1, float u2, float v2, float texWidth, float texHeight, boolean mirrorIn, Vec3f directionIn) {
			this.vertexPositions = posIn;
			int m = v1 > v2 ? -1 : 1;
			//Extend to fix precision issue
			posIn[0] = posIn[0].setTextureUV(u2 / texWidth - 0.001f, v1 / texHeight + 0.001f * m);
			posIn[1] = posIn[1].setTextureUV(u1 / texWidth + 0.001f, v1 / texHeight + 0.001f * m);
			posIn[2] = posIn[2].setTextureUV(u1 / texWidth + 0.001f, v2 / texHeight - 0.001f * m);
			posIn[3] = posIn[3].setTextureUV(u2 / texWidth - 0.001f, v2 / texHeight - 0.001f * m);
			if (mirrorIn) {
				int i = posIn.length;
