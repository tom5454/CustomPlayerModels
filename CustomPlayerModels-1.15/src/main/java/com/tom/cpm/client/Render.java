...
public class Render {
	...
	public static void drawBoundingBox(MatrixStack matrixStackIn, IVertexBuilder buffer, float x, float y, float z,
			float w, float h, float d, float r, float g, float b, float a) {
		WorldRenderer.drawBoundingBox(matrixStackIn, buffer, x, y, z, x+w, y+h, z+d, r, g, b, a);
	}
	...
	public static class ColorBox implements Box {
		...
		@Override
		public void draw(MatrixStack matrixStackIn, IVertexBuilder bufferIn, IRenderTypeBuffer buf, int packedLightIn,
				int packedOverlayIn, float red, float green, float blue, float alpha) {
			Matrix4f matrix4f = matrixStackIn.getLast().getMatrix();
			Matrix3f matrix3f = matrixStackIn.getLast().getNormal();

			IVertexBuilder buffer = buf.getBuffer(CustomRenderTypes.getEntityColorTranslucentCull());
			...
			for(TexturedQuad modelrenderer$texturedquad : quadList) {
				...
				for(int i = 0; i < 4; ++i) {
					...
					buffer.addVertex(vector4f.getX(), vector4f.getY(), vector4f.getZ(), red, green, blue, alpha, modelrenderer$positiontexturevertex.textureU, modelrenderer$positiontexturevertex.textureV, packedOverlayIn, packedLightIn, f, f1, f2);
				}
			}
		}
	...
	public static interface Box {
		public void draw(MatrixStack matrixStackIn, IVertexBuilder bufferIn, IRenderTypeBuffer buf, int packedLightIn, int packedOverlayIn, float red, float green, float blue, float alpha);
	}
	...
	public static void drawOrigin(MatrixStack matrixStackIn, IVertexBuilder bufferIn, float len) {
		Matrix4f matrix4f = matrixStackIn.getLast().getMatrix();
		bufferIn.pos(matrix4f, 0, 0, 0).color(0f, 1, 0, 1).endVertex();
		bufferIn.pos(matrix4f, 0, len, 0).color(0f, 1, 0, 1).endVertex();
		bufferIn.pos(matrix4f, 0, 0, 0).color(1f, 0, 0, 1).endVertex();
		bufferIn.pos(matrix4f, len, 0, 0).color(1f, 0, 0, 1).endVertex();
		bufferIn.pos(matrix4f, 0, 0, 0).color(0f, 0, 1, 1).endVertex();
		bufferIn.pos(matrix4f, 0, 0, len).color(0f, 0, 1, 1).endVertex();
	}
}
