...
public class GuiImpl extends Screen implements IGui {
	...
	public void render(int mouseX, int mouseY, float partialTicks) {
		...
		if(minecraft.player != null) {
			try {
				...
				m.invoke(minecraft.ingameGUI, minecraft.getMainWindow().getScaledWidth(), minecraft.getMainWindow().getScaledHeight());
			} catch (Throwable e) {
			}
		}
	}
	...
	private void displayError(String e) {
		Screen p = parent;
		parent = null;
		Minecraft.getInstance().displayGuiScreen(new ErrorScreen(new StringTextComponent("Custom Player Models"), I18n.format("error.cpm.crash", e)) {
	...
	public void drawGradientBox(int x, int y, int w, int h, int topLeft, int topRight, int bottomLeft,
		...
		bufferbuilder.pos(right, top, this.getBlitOffset()).color(rtr, gtr, btr, atr).endVertex();
		bufferbuilder.pos(left, top, this.getBlitOffset()).color(rtl, gtl, btl, atl).endVertex();
		bufferbuilder.pos(left, bottom, this.getBlitOffset()).color(rbl, gbl, bbl, abl).endVertex();
		bufferbuilder.pos(right, bottom, this.getBlitOffset()).color(rbr, gbr, bbr, abr).endVertex();
		tessellator.draw();
