...
public class BiomeHandlerImpl extends BiomeHandler<Holder<Biome>> {
	...
	public List<com.tom.cpl.block.Biome> listNativeEntries(String tag) {
		...
		Holder<Biome> b = registry.get().getHolder(ResourceKey.create(Registry.BIOME_REGISTRY, rl)).orElse(null);
	...
	public boolean isInTag(String tag, Holder<Biome> state) {
		if (tag.charAt(0) == '#') {
			...
			if (rl != null) {
				TagKey<Biome> i = TagKey.create(Registry.BIOME_REGISTRY, rl);
				return state.is(i);
			}
		} else {
