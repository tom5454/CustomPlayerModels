	...
	public static void init() {
		...
		transformers.put("net.minecraft.client.renderer.entity.RenderPlayer", new UnaryOperator<ClassNode>() {
			...
			public ClassNode apply(ClassNode input) {
				...
				inst.add(new VarInsnNode(Opcodes.FLOAD, 3));
				inst.add(new VarInsnNode(Opcodes.FLOAD, 4));
				inst.add(new VarInsnNode(Opcodes.FLOAD, 5));
				inst.add(new VarInsnNode(Opcodes.FLOAD, 6));
				inst.add(new VarInsnNode(Opcodes.FLOAD, 7));
				inst.add(new MethodInsnNode(Opcodes.INVOKESTATIC, HOOKS_CLASS, "onRenderPlayerModel", "(Lnet/minecraft/client/renderer/entity/RenderPlayer;Lnet/minecraft/entity/EntityLivingBase;FFFFFF)Z", false));
				LabelNode lbln = new LabelNode();
				inst.add(new JumpInsnNode(Opcodes.IFEQ, lbln));
				...
				for (MethodNode method : input.methods) {
					if (method.name.equals(name)) {
						LOG.info("CPM Render Invis Hook: Found existing method");
						mn = method;
						break;
					}
				}
				if (mn == null) {
					mn = new MethodNode(Opcodes.ACC_PUBLIC, name, "(Lnet/minecraft/entity/EntityLivingBase;FFFFFF)V", null, null);
					mn.instructions.add(inst);
					...
					mn.instructions.insertBefore(mn.instructions.getFirst(), inst);
				}
