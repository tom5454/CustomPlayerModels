...
public class PlayerInventory implements Inventory {
	...
	public int getNamedSlotId(NamedSlot slot) {
		return switch (slot) {
		case MAIN_HAND -> inv.getSelectedSlot();
		case ARMOR_BOOTS -> EquipmentSlot.FEET.getIndex(inv.getNonEquipmentItems().size());
		case ARMOR_CHESTPLATE -> EquipmentSlot.CHEST.getIndex(inv.getNonEquipmentItems().size());
		case ARMOR_HELMET -> EquipmentSlot.HEAD.getIndex(inv.getNonEquipmentItems().size());
		case ARMOR_LEGGINGS -> EquipmentSlot.LEGS.getIndex(inv.getNonEquipmentItems().size());
		case OFF_HAND -> inv.getNonEquipmentItems().size() + 4;
