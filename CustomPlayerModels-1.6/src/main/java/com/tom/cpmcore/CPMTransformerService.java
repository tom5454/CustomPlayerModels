...
public class CPMTransformerService implements IClassTransformer {
	...
	public static void init() {
		...
		try (BufferedReader rd = new BufferedReader(new InputStreamReader(CPMTransformerService.class.getResourceAsStream("/META-INF/cpm_core.cfg")))) {
			...
			while ((ln = rd.readLine()) != null) {
				...
				if (sp[0].equals("method")) {
					String nameDesc = sp[2].split("/", 2)[1];
		...
		transformers.put("net.minecraft.client.renderer.entity.RendererLivingEntity", new UnaryOperator<ClassNode>() {
			...
			@Override
			public ClassNode apply(ClassNode input) {
				Pair<String, String> mth = lookupMethod("RendererLivingEntity;doRenderLiving(Lnet.minecraft.entity.EntityLivingBase;DDDFF)V");
				Pair<String, String> mth2 = lookupMethod("ModelBase;render(Lnet.minecraft.entity.Entity;FFFFFF)V");
				Pair<String, String> mth3 = lookupMethod("RendererLivingEntity;passSpecialRender(Lnet.minecraft.entity.EntityLivingBase;DDD)V");
				...
				for(MethodNode m : (List<MethodNode>) input.methods) {
					if(checkMethod(m, mth) || m.name.equals("renderPlayer")) {
						...
						lst.add(new MethodInsnNode(Opcodes.INVOKESTATIC, HOOKS_CLASS, "onRenderName", "(Lnet/minecraft/client/renderer/entity/RendererLivingEntity;Lnet/minecraft/entity/EntityLivingBase;DDD)Z"));
						LabelNode lbln = new LabelNode();
						lst.add(new JumpInsnNode(Opcodes.IFEQ, lbln));
						lst.add(new InsnNode(Opcodes.RETURN));
						lst.add(lbln);
		...
		transformers.put("net.minecraft.client.renderer.entity.RenderPlayer", new UnaryOperator<ClassNode>() {
			...
			public ClassNode apply(ClassNode input) {
				...
				Pair<String, String> mth = lookupMethod("RenderPlayer;renderSpecials(Lnet.minecraft.client.entity.AbstractClientPlayer;F)V");
						...
						for (ListIterator<AbstractInsnNode> it = method.instructions.iterator(); it.hasNext(); ) {
							...
							if(insnNode instanceof FieldInsnNode) {
								...
								if(mn.owner.equals("net/minecraftforge/client/event/RenderPlayerEvent$Specials$Pre") && mn.name.equals("renderCape")) {
									...
									lst.add(new MethodInsnNode(Opcodes.INVOKESTATIC, HOOKS_CLASS, "renderCape", "(ZLnet/minecraft/client/renderer/entity/RenderPlayer;Lnet/minecraft/client/entity/AbstractClientPlayer;F)Z"));
				...
				mn.instructions.add(new MethodInsnNode(Opcodes.INVOKESTATIC, HOOKS_CLASS, "onRenderPlayerModel", "(Lnet/minecraft/client/renderer/entity/RenderPlayer;Lnet/minecraft/entity/EntityLivingBase;FFFFFF)Z"));
				LabelNode lbln = new LabelNode();
				...
				mn.instructions.add(new VarInsnNode(Opcodes.FLOAD, 6));
				mn.instructions.add(new VarInsnNode(Opcodes.FLOAD, 7));
				mn.instructions.add(new MethodInsnNode(Opcodes.INVOKESPECIAL, "net/minecraft/client/renderer/entity/RendererLivingEntity", name, "(Lnet/minecraft/entity/EntityLivingBase;FFFFFF)V"));
				mn.instructions.add(new InsnNode(Opcodes.RETURN));
				mn.maxLocals = 8;
				mn.maxStack = 8;
		...
		transformers.put("net.minecraft.client.renderer.ThreadDownloadImageData", new UnaryOperator<ClassNode>() {

			@Override
			public ClassNode apply(ClassNode input) {
				for(MethodNode method : (List<MethodNode>) input.methods) {
					if (method.name.equals("getImageUrl") || method.name.equals("func_110554_a")) {
						LOG.info("CPM Skin Fixer Hook: Found getImageUrl");
