...
public class BiomeHandlerImpl extends BiomeHandler<BiomeGenBase> {
	...
	public List<com.tom.cpl.block.Biome> listNativeEntries(String tag) {
		BiomeGenBase b = Arrays.stream(BiomeGenBase.biomeList).filter(e -> e.biomeName.equals(tag)).findFirst().orElse(null);
	...
	public List<com.tom.cpl.block.Biome> getAllElements() {
		return Arrays.stream(BiomeGenBase.biomeList).map(this::wrap).collect(Collectors.toList());
	}
