...
public class GuiImpl extends GuiScreen implements IGui {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		try {
			...
			String s = "Minecraft 1.10.2 (" + this.mc.getVersion() + "/" + ClientBrandRetriever.getClientModName() + ") " + Loader.instance().getIndexedModList().get(CustomPlayerModels.ID).getDisplayVersion();
	...
	protected void keyTyped(char typedChar, int keyCode) throws IOException {
		try {
			...
			gui.keyPressed(evt);
			if(!evt.isConsumed()) {
				if(gui.enableChat() && mc.thePlayer != null && mc.gameSettings.keyBindChat.getKeyCode() == keyCode && mc.gameSettings.chatVisibility != EntityPlayer.EnumChatVisibility.HIDDEN) {
	...
	public void drawFormattedText(float x, float y, IText text, int color, float scale) {
		...
		fontRendererObj.drawString(text.<ITextComponent>remap().getFormattedText(), 0, 0, color);
		GlStateManager.popMatrix();
	}

	@Override
	public int textWidthFormatted(IText text) {
		return fontRendererObj.getStringWidth(text.<ITextComponent>remap().getFormattedText());
	}
