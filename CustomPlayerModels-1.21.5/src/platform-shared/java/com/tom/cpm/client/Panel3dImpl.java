...
public class Panel3dImpl extends Panel3dNative {
	...
	public void render(float partialTicks) {
		...
		guiGraphics.flush();
		RenderTarget renderTarget = this.mc.getMainRenderTarget();
		RenderSystem.getDevice().createCommandEncoder().clearDepthTexture(renderTarget.getDepthTexture(), 1.0);

		RenderSystem.backupProjectionMatrix();
		Box bounds = getBounds();
		//Matrix4f matrix4f = mc.gameRenderer.getProjectionMatrix(45f);
		...
		try {
			...
			guiGraphics.pose().translate(off.x + bounds.w / 2, off.y + bounds.h / 2, 300.0f);
