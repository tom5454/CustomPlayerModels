...
public class BlockStateHandlerImpl extends BlockStateHandler<IBlockState> {
	...
	public String getBlockId(IBlockState stack) {
		return stack.getBlock().delegate.getResourceName().toString();
	}

	@Override
	public List<String> getBlockStates(IBlockState stack) {
		return stack.getPropertyNames().stream().map(p -> p.getName()).collect(Collectors.toList());
	}
	...
	public List<String> getAllValuesFor(IBlockState state, String property) {
		return (List<String>) getProp(state, property).flatMap(e -> e.getKey().getAllowedValues().stream().map(v -> {
			IProperty p = e.getKey();
	...
	public List<BlockState> listNativeEntries(String tag) {
		if (tag.charAt(0) == '#') {
			...
			Block item = GameData.getBlockRegistry().getObject(rl);
			if (item != null) {
	...
	public List<BlockState> getAllElements() {
		...
		while (itr.hasNext()) {
			...
			list.add(wrap(block.getDefaultState()));
		}
		return list;
	}
