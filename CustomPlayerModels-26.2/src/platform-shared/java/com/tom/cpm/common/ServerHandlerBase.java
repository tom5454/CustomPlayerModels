...
public class ServerHandlerBase {
	...
	public static NetHandler<CustomPacketPayload.Type<ByteArrayPayload>, ServerPlayer, ServerGamePacketListenerImpl> init() {
		NetHandler<CustomPacketPayload.Type<ByteArrayPayload>, ServerPlayer, ServerGamePacketListenerImpl> netHandler = new NetHandler<>((k, v) -> new CustomPacketPayload.Type<>(Identifier.tryBuild(k, v)));
		...
		netHandler.setSendPacketServer(Function.identity(), (c, rl, pb) -> c.send(new ClientboundCustomPayloadPacket(new ByteArrayPayload(rl, pb))), ent -> {
			ChunkMap.TrackedEntity tr = ent.level().getChunkSource().chunkMap.entityMap.get(ent.getId());
			if(tr != null) {
				return tr.seenBy;
			}
		...
		netHandler.setFindTracking((p, f) -> {
			for(ChunkMap.TrackedEntity tr : p.level().getChunkSource().chunkMap.entityMap.values()) {
				if(tr.entity instanceof Player && tr.seenBy.contains(p.connection)) {
					f.accept((ServerPlayer) tr.entity);
				}
			}
		});
		netHandler.setSendChat((p, m) -> p.sendSystemMessage(m.remap(), false));
		...
		netHandler.setKickPlayer((p, m) -> p.connection.disconnect(new DisconnectionDetails(m.remap())));
