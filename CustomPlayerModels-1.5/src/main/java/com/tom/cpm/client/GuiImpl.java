...
public class GuiImpl extends GuiScreen implements IGui {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		try {
			...
			String s = "Minecraft 1.5.2 (" + ClientBrandRetriever.getClientModName() + ") " + Loader.instance().getIndexedModList().get(CustomPlayerModels.ID).getDisplayVersion();
	...
	public void displayError(String e) {
		GuiScreen p = parent;
		parent = null;
		Minecraft.getMinecraft().displayGuiScreen(new GuiErrorScreen("Custom Player Models", Lang.format("error.cpm.crash", e)) {
	...
	public void drawTexture(int x, int y, int w, int h, int u, int v, String texture) {
		mc.renderEngine.bindTexture("/assets/cpm/textures/gui/" + texture + ".png");
		x += getOffset().x;
		y += getOffset().y;
	...
	public void drawStack(int x, int y, Stack stack) {
		...
		itemRenderer.renderItemOverlayIntoGUI(this.fontRenderer, this.mc.renderEngine, s, x, y, null);
