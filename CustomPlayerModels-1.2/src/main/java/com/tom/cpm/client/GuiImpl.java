...
public class GuiImpl extends GuiScreen implements IGui {
	...
	protected void keyTyped(char typedChar, int keyCode) {
		try {
			...
			gui.keyPressed(evt);
			if(!evt.isConsumed()) {
				if(mc.thePlayer != null && mc.gameSettings.keyBindChat.keyCode == keyCode) {
	...
	private class TxtField implements ITextField {
		...
		public TxtField() {
			...
			this.field.enabledColor = colors.label_text_color;
		}
	...
	protected void drawItemStackTooltip(ItemStack par1ItemStack, int par2, int par3) {
		List<String> list = par1ItemStack.getItemNameandInformation();
